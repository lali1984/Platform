...
const user_profile_1 = require("../../domain/aggregates/user-profile");
...
class GetUserProfileUseCase {
    ...
    async execute(token, userId) {
        try {
            ...
            // 5. Create aggregate
            const userProfile = user_profile_1.UserProfileAggregate.create(validationResult.user, completeProfile, // Теперь передаем CompleteUserProfile
            `session_${Date.now()}`);
            // 6. Cache the result
