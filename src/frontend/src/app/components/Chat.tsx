...
export function Chat({ isCollapsed, setIsCollapsed, isOnLeft, setIsOnLeft }: ChatProps) {
  ...
  const [expandedSection, setExpandedSection] = useState<'messenger' | 'contacts' | 'notifications' | 'ai' | null>(null);
  const { t } = useLanguage();
  ...
  const handleSend = () => {
    if (message.trim()) {
      const newMessage: Message = {
        ...
        textKey: message, // For user messages, we store the actual text
        sender: 'user',
                  ...
                  {initialMessages.map((message) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          {message.sender === 'other' && message.userNameKey && (
                            <p className="text-xs text-gray-600 dark:text-gray-400 mb-1 px-2">{t(message.userNameKey)}</p>
                          )}
                          <div
                          ...
                          >
                            <p className="text-sm">{message.sender === 'user' ? message.textKey : t(message.textKey)}</p>
                          </div>
                          <p className="text-xs text-gray-500 dark:text-gray-500 mt-1 px-2">{message.time}</p>
                        </div>
                      </div>
                    </div>
                  ))}
