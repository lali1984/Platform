...
sequenceDiagram
    ...
    A->>A: Валидация, создание User entity, сохранение в auth_db.users
    A->>A: Публикация createUserRegisteredEvent в outbox_events (транзакционно)
    A-->>B: Успешный ответ с tokens и userId
    B->>R: (Ожидание) Слушает топик user.events через KafkaConsumer
    R->>K: Периодически читает из auth_db.outbox_events и публикует в Kafka
    K->>R: Событие UserRegisteredEvent отправлено в топик auth.events.registered.v1
    R->>U: (Реакция) user-service может подписаться на auth.events.registered.v1 и создать профиль
    R->>B: KafkaConsumer в bff-gateway получает UserCreatedEvent из user.events.created.v1
    B->>U: GET /users/{userId} (HTTP) для получения профиля
    U->>U: Поиск по ID в user_db.user_profiles
    U-->>B: Возвращает UserProfile
    B->>F: Успешный ответ с tokens + profile
```
...
sequenceDiagram
    ...
    B->>C: GET user:{userId}:profile (проверка кэша)
    C-->>B: null (кэш промах)
    B->>A: POST /login {email, password} (HTTP)
    A->>A: Поиск по email, валидация пароля, обновление lastLoginAt
    A->>A: Публикация UserLoggedIn в outbox_events
    A-->>B: Успешный ответ с tokens и userId
    B->>U: GET /users/profile/{userId} (HTTP) с токеном авторизации
    U->>U: Поиск по ID в user_db.user_profiles
    U-->>B: Возвращает UserProfile
    B->>C: SET user:{userId}:profile (сохранение в кэш на 5 мин)
    B-->>F: Успешный ответ с tokens + profile
```
...
sequenceDiagram
    ...
    U->>U: Поиск по ID, обновление в user_db.user_profiles
    U->>U: Публикация UserUpdatedEvent в outbox_events
    U-->>B: Успешный ответ с обновлённым профилем
    B-->>F: Успешный ответ
```
